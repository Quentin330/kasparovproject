...
public class Pawn extends Piece implements Serializable {
	...
	/**
	 * Retourne la liste des coups possibles du pion (sans prendre en
	...
	public ArrayList<Square> possibleMoves(Board board) {
		...
		if (this.isBlack()) {
			if (this.getRow() == 7) {
				...
				if (this.getColumn() != 1
						&& board.isWhite(6, this.getColumn() - 1))
				...
				if (this.getRow() != 1
						&& board.isEmpty(this.getRow() - 1, this.getColumn()))
					movesList.add(new Square(this.getRow() - 1, this
							.getColumn()));
				if (this.getRow() != 1
						&& this.getColumn() != 1
						&& board.isWhite(this.getRow() - 1,
								this.getColumn() - 1))
					movesList.add(new Square(this.getRow() - 1, this
							.getColumn() - 1));
				if (this.getRow() != 1
			...
			if (this.getRow() == 2) {
				...
				if (this.getColumn() != 1
						&& board.isBlack(3, this.getColumn() - 1))
		...
		if ((this.getColor().equals("black") && this.getRow() == 4)
			...
			if (this.getColumn() != 1) {
				if (!board.isEmpty(this.getRow(), this.getColumn() - 1)) {
					Piece p = board.getPiece(this.getRow(),
							this.getColumn() - 1);
					if (p instanceof Pawn) {
						if (p.getMangeableEnPrisePassant() == board
								.getNumeroCoup()) {
							if (this.getColor().equals("black")
									&& board.isEmpty(3, this.getColumn() - 1))
								movesList.add(new Square(3,
										this.getColumn() - 1));
							else if (this.getColor().equals("white")
									&& board.isEmpty(6, this.getColumn() - 1))
								movesList.add(new Square(6,
										this.getColumn() - 1));
						}
					}
				}

...
public class King extends Piece implements Serializable {
	...
	/**
	 * Retourne la liste des coups possibles du roi (sans prendre en
	 * considération les coups qui peuvent le mettre en échec).
	 */
	...
	/**
	 * Retourne le booleen remplissant les conditions nécessaires pour effectuer
	 * le petit roque.
	 * 
	 * @param board
	 *            La partie en cours.
	 ...
	 * @return Le booleen remplissant les conditions nécessaires pour effectuer
	 *         le petit roque.
	 */
	private boolean canShortCastle(Board board, int row) {
		...
		return this.isSameColor(board, row, 8)
				...
				&& board.isEmpty(row, 6) && board.isEmpty(row, 7)
	...
	private boolean canLongCastle(Board board, int row) {
		...
		return this.isSameColor(board, row, 1)
				...
				&& board.isEmpty(row, 4) && board.isEmpty(row, 3)
				&& board.isEmpty(row, 2)
				&& !board.isEchec(this.getColor(), row, 5)
